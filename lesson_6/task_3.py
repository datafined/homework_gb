...
class Position(Worker):
    def get_full_name(self):
        name_full = f'Полное имя сотрудника {self.name}, {self.surname}, должность - {self.position}'
        return name_full

    def get_total_income(self):
        tot_income = f'Доход {self._income.get("wage") + self._income.get("bonus")}'
        return tot_income


answer = (Position('ivan', 'ivanov', 'director', 100, 50))
print(answer.get_full_name())
print(answer.get_total_income())
